...
namespace Asmodat.Abbreviate
{
    public partial class DataBuffer
    {
        ...
        ThreadedDictionary<DateTime, string> DDTSBuffer;
        ...
        /// <summary>
        /// This is data buffer, that consist of Dictionary of DataTime and string data, where data is in conjuicntion with time in relation to Set Trime.
        /// </summary>
        public DataBuffer()
        {
            DDTSBuffer = new ThreadedDictionary<DateTime, string>();
        }
        ...
        public void Set(string sData)
        {
            lock (Locker.Get("DDTSBuffer"))
            {
                ...
                else DDTSBuffer.Add(DateTime.Now, sData);
            }
        }
        ...
        /// <returns></returns>
        public string Get(DateTime DTime, bool bIncrement = false)
        {
        ...
        public KeyValuePair<DateTime, string> GetPair(bool increment = false)
        {
            if (Indexer < 0) 
                return new KeyValuePair<DateTime, string>(DateTime.MinValue, null);

            DateTime key = this.GetKey(Indexer);
            string value = this.GetValue(Indexer);
            var kvp = new KeyValuePair<DateTime, string>(key, value);

            if(increment) this.IndexerIcrement();
        ...
        public DateTime GetKey(int iIndex)
        {
            if (this.Count <= iIndex) return default(DateTime);
        ...
        /// <returns></returns>
        public bool IsValid(DateTime DTKey)
        {
            if (DTKey.Date == default(DateTime).Date) return false;
            else return true;
        }
